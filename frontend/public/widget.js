"use strict";(()=>{var R="2.1.0",K="https://webtalk-ai.onrender.com",V="wss://webtalk-ai.onrender.com";console.log("%c[WebTalkAI v"+R+"] %cText = REST \xB7 Voice = WebSocket (mic only)","background:#6366f1;color:#fff;padding:2px 6px;border-radius:3px;font-weight:bold","color:#6b7280");var j={purple:{primary:"#6366f1",accent:"#a855f7"},blue:{primary:"#3b82f6",accent:"#06b6d4"},green:{primary:"#10b981",accent:"#84cc16"},dark:{primary:"#1f2937",accent:"#6366f1"}},o,h=null,p=[],$,v=!1,T=!1,c=null,w=null,M=null,C=!1,f=null,y,l,S,x,B,z,U,P,u;function N(){try{let e="wtai_session_v2",t=localStorage.getItem(e);return t||(t=`s_${Math.random().toString(36).slice(2,11)}_${Date.now()}`,localStorage.setItem(e,t)),t}catch(e){return`s_${Date.now()}`}}function k(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}function F(e){try{let t=new URL(e),i=t.pathname.length>18?t.pathname.slice(0,18)+"\u2026":t.pathname;return t.hostname.replace(/^www\./,"")+(i==="/"?"":i)}catch(t){return e.slice(0,30)}}function J(){let e=o.theme,t=o.position==="bottom-right"?"right":"left",i=`
...
@media (max-width: 480px) {
  ...
  `.trim(),a=document.createElement("style");a.id="wtai-styles",a.textContent=i,document.head.appendChild(a)}function X(){y=document.createElement("button"),y.className="wtai-w wtai-launcher",y.setAttribute("aria-label","Open chat"),y.innerHTML='<svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12c0 1.86.5 3.6 1.38 5.1L2 22l4.9-1.38C8.4 21.5 10.14 22 12 22c5.52 0 10-4.48 10-10S17.52 2 12 2zm0 18c-1.65 0-3.27-.45-4.7-1.3l-.34-.2-3.04.86.85-3.03-.21-.34C3.45 14.27 3 12.65 3 11c0-4.97 4.03-9 9-9s9 4.03 9 9-4.03 9-9 9z"/><circle cx="8" cy="12" r="1.4"/><circle cx="12" cy="12" r="1.4"/><circle cx="16" cy="12" r="1.4"/></svg>',y.onclick=_,document.body.appendChild(y),l=document.createElement("div"),l.className="wtai-w wtai-panel",l.innerHTML=`
    <div class="wtai-head">
      ...
      <div style="flex:1;min-width:0;">
        <div class="wtai-name">${k((h==null?void 0:h.company_name)||"AI Assistant")}</div>
  ...
  `,document.body.appendChild(l),S=l.querySelector(".wtai-msgs"),x=l.querySelector(".wtai-in"),z=l.querySelector(".wtai-send"),B=l.querySelector(".wtai-mic"),U=l.querySelector(".wtai-sugs"),P=l.querySelector(".wtai-voice"),u=l.querySelector(".wtai-vstat"),l.querySelector(".wtai-close").onclick=_,z.onclick=()=>W(),x.addEventListener("keydown",e=>{e.key==="Enter"&&!T&&W()}),B&&(B.onclick=Z),l.querySelector('[data-act="cancel"]').onclick=()=>A(!0),l.querySelector('[data-act="stop"]').onclick=()=>A(!1)}function _(){v=!v,l.classList.toggle("open",v),v&&(x.focus(),setTimeout(()=>S.scrollTo({top:S.scrollHeight,behavior:"smooth"}),50))}function E(){S.innerHTML=p.map(e=>{let t='<svg viewBox="0 0 24 24"><path d="M20 9V7c0-1.1-.9-2-2-2h-3c0-1.66-1.34-3-3-3S9 3.34 9 5H6c-1.1 0-2 .9-2 2v2c-1.66 0-3 1.34-3 3s1.34 3 3 3v4c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2v-4c1.66 0 3-1.34 3-3s-1.34-3-3-3zM7.5 11.5c0-.83.67-1.5 1.5-1.5s1.5.67 1.5 1.5S9.83 13 9 13s-1.5-.67-1.5-1.5zM16 17H8v-2h8v2zm-1-4c-.83 0-1.5-.67-1.5-1.5S14.17 10 15 10s1.5.67 1.5 1.5S15.83 13 15 13z"/></svg>',i='<svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>',a=e.sources&&e.sources.length&&!e.streaming?`<div class="wtai-srcs">${e.sources.slice(0,4).map(m=>`<a class="wtai-src" href="${k(m)}" target="_blank" rel="noopener">${k(F(m))}</a>`).join("")}</div>`:"";if(e.role==="user")return`<div class="wtai-msg user"><div class="wtai-bub">${k(e.content)}</div></div>`;let s=e.error?"wtai-msg assistant err":"wtai-msg assistant",b=e.error?"wtai-mbot err":"wtai-mbot",d=e.streaming&&e.content?'<span class="wtai-caret"></span>':"",n=e.streaming&&!e.content?'<div class="wtai-typing"><span></span><span></span><span></span></div>':`${k(e.content)}${d}`;return`<div class="${s}">
      <div class="${b}">${e.error?i:t}</div>
      <div style="max-width:75%"><div class="wtai-bub">${n}</div>${a}</div>
    </div>`}).join(""),S.scrollTo({top:S.scrollHeight,behavior:"smooth"})}function O(){if(p.length>1||T){U.innerHTML="";return}let e=["What is this site about?","What do you offer?","How can I get in touch?"];U.innerHTML=e.map(t=>`<button class="wtai-sug">${k(t)}</button>`).join(""),U.querySelectorAll(".wtai-sug").forEach((t,i)=>{t.onclick=()=>W(e[i])})}var D=6e4;async function W(e){var b,d;let t=(e!=null?e:x.value).trim();if(!t||T)return;x.value="",p.push({role:"user",content:t}),p.push({role:"assistant",content:"",streaming:!0}),T=!0,z.disabled=!0,x.disabled=!0,E(),O();let i=new AbortController,a=setTimeout(()=>{console.warn("[WebTalkAI] Request timed out after",D,"ms"),i.abort()},D),s=performance.now();console.log("[WebTalkAI] \u2192 REST POST /widget/chat",{message:t,conversation_id:$});try{let n=await fetch(`${o.apiUrl}/api/v1/widget/chat`,{method:"POST",headers:{"Content-Type":"application/json","X-API-Key":o.apiKey},body:JSON.stringify({api_key:o.apiKey,message:t,conversation_id:$}),signal:i.signal});clearTimeout(a);let m=Math.round(performance.now()-s);if(console.log("[WebTalkAI] \u2190 status",n.status,"in",m,"ms"),!n.ok){let H="";try{H=await n.text()}catch(Q){}console.error("[WebTalkAI] HTTP",n.status,H);let L=`Server returned ${n.status}.`;throw n.status===401?L="Invalid API key.":n.status===404?L="Endpoint not found.":n.status===429?L="Too many requests. Please wait a moment.":n.status>=500&&(L="The AI is having trouble. Please try again."),new Error(L)}let r=await n.json();console.log("[WebTalkAI] response:",{len:(b=r.response)==null?void 0:b.length,sources:(d=r.sources)==null?void 0:d.length});let g=r.response||r.answer||"I couldn't generate a response. Please try again.",q=r.sources||[],I=p[p.length-1];I&&(I.content=g,I.sources=q,I.streaming=!1),E(),o.ttsAutoPlay&&g&&Y(g).catch(H=>console.warn("[WebTalkAI] TTS failed:",H))}catch(n){clearTimeout(a);let m=Math.round(performance.now()-s);console.error("[WebTalkAI] Chat failed after",m,"ms:",n);let r="Something went wrong. Please try again.";n instanceof DOMException&&n.name==="AbortError"?r="The AI is starting up \u2014 please wait a moment and try again. (The server sleeps when idle on free hosting.)":n instanceof TypeError&&n.message==="Failed to fetch"?r="Couldn't reach the server. Please check your connection.":n instanceof Error&&(r=n.message);let g=p[p.length-1];g&&g.streaming?(g.content=r,g.streaming=!1,g.error=!0):p.push({role:"assistant",content:r,error:!0}),E()}finally{T=!1,z.disabled=!1,x.disabled=!1,x.focus()}}async function Y(e){try{f&&(f.pause(),f=null);let t=await fetch(`${o.apiUrl}/api/v1/widget/tts`,{method:"POST",headers:{"Content-Type":"application/json","X-API-Key":o.apiKey},body:JSON.stringify({text:e})});if(!t.ok)return;let i=await t.blob(),a=URL.createObjectURL(i);f=new Audio(a),f.onended=()=>URL.revokeObjectURL(a),await f.play()}catch(t){console.warn("[WebTalkAI] TTS playback failed:",t)}}async function Z(){if(!o.voiceEnabled||!h||T)return;P.classList.add("active"),u.textContent="Connecting\u2026",C=!1;try{M=await navigator.mediaDevices.getUserMedia({audio:!0})}catch(i){u.textContent="Microphone access denied",setTimeout(()=>A(!0),1500);return}let e=`${o.wsUrl}/ws/voice/${h.client_id}?session_id=${encodeURIComponent($)}&api_key=${encodeURIComponent(o.apiKey)}`;try{c=new WebSocket(e)}catch(i){u.textContent="Connection failed",setTimeout(()=>A(!0),1500);return}let t=[];c.onopen=()=>{u.textContent="Listening\u2026",w=new MediaRecorder(M,{mimeType:"audio/webm"}),w.ondataavailable=i=>{if(!c||c.readyState!==WebSocket.OPEN||i.data.size===0)return;let a=new FileReader;a.onload=()=>{let s=a.result.split(",")[1];c.send(JSON.stringify({type:"audio_chunk",data:s}))},a.readAsDataURL(i.data)},w.start(300)},c.onmessage=i=>{let a=JSON.parse(i.data);if(a.type==="transcript"&&a.text)p.push({role:"user",content:a.text}),E();else if(a.type==="answer_text")p.push({role:"assistant",content:a.text}),u.textContent="Speaking\u2026",E();else if(a.type==="audio_chunk"){let s=atob(a.data),b=new Uint8Array(s.length);for(let d=0;d<s.length;d++)b[d]=s.charCodeAt(d);t.push(b)}else if(a.type==="audio_end"){let s=t.reduce((r,g)=>r+g.length,0),b=new Uint8Array(s),d=0;for(let r of t)b.set(r,d),d+=r.length;t.length=0;let n=new Blob([b],{type:"audio/mpeg"}),m=URL.createObjectURL(n);f&&f.pause(),f=new Audio(m),f.onended=()=>{URL.revokeObjectURL(m),C||(u.textContent="Listening\u2026",w&&w.state==="inactive"&&w.start(300))},f.play().catch(()=>{})}else a.type==="error"&&(u.textContent="Error: "+a.message,setTimeout(()=>A(!0),1800))},c.onerror=()=>{u.textContent="Voice connection error"},c.onclose=()=>{C||(u.textContent="Disconnected")}}function A(e){C=e,P.classList.remove("active"),w&&w.state!=="inactive"&&w.stop(),!e&&c&&c.readyState===WebSocket.OPEN?(u.textContent="Thinking\u2026",c.send(JSON.stringify({type:"audio_end"}))):c&&(c.close(),c=null),M&&(M.getTracks().forEach(t=>t.stop()),M=null),w=null}async function G(e){if(!e||!e.apiKey){console.error("[WebTalkAI] apiKey is required");return}if(document.getElementById("wtai-styles")){console.warn("[WebTalkAI] Already initialized");return}let t=e.theme&&j[e.theme]?e.theme:"purple";o={apiKey:e.apiKey,apiUrl:e.apiUrl||K,wsUrl:e.wsUrl||V,position:e.position==="bottom-left"?"bottom-left":"bottom-right",theme:j[t],greeting:e.greeting||"Hi! How can I help you today?",voiceEnabled:e.voiceEnabled!==!1,ttsAutoPlay:e.ttsAutoPlay===!0},$=N(),console.log("[WebTalkAI] init v"+R,{apiUrl:o.apiUrl,wsUrl:o.wsUrl,theme:e.theme||"purple",position:o.position,voiceEnabled:o.voiceEnabled,sessionId:$});let i=performance.now();try{let a=await fetch(`${o.apiUrl}/api/v1/widget/config?api_key=${encodeURIComponent(o.apiKey)}`),s=Math.round(performance.now()-i);if(!a.ok){console.error(`[WebTalkAI] Bootstrap failed: HTTP ${a.status} after ${s}ms. Check that the API key is valid and that ${o.apiUrl} is reachable.`);return}h=await a.json(),console.log("[WebTalkAI] connected in",s,"ms \u2014 tenant:",h==null?void 0:h.company_name)}catch(a){console.error("[WebTalkAI] Bootstrap failed (network):",a);return}J(),X(),p=[{role:"assistant",content:o.greeting}],E(),O()}window.WebTalkAI={init:G,open:()=>{v||_()},close:()=>{v&&_()},version:R};})();
