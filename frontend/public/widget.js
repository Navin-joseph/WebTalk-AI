...
(() => {
  ...
  function sid() {
    try {
      let s = localStorage.getItem("wtai_sid4");
      if (!s) { s = "s_" + Math.random().toString(36).slice(2) + "_" + Date.now(); localStorage.setItem("wtai_sid4", s); }
      return s;
    } catch { return "s_" + Date.now(); }
  }

  function setStatus(t) { if (statusEl) statusEl.textContent = t; }
  function setActive(_on) { /* compat stub */ }

  const AV_STATES  = ["av-idle","av-thinking","av-listening","av-speaking"];
  const STATUS_TXT = { idle:"Ask about this site", thinking:"Thinking…", listening:"Listening…", speaking:"Speaking…" };
  ...
  function setAvatarState(state) {
    ...
    if (sub) sub.textContent = STATUS_TXT[state] || "";

    // CSS animations on <video> avatars
    if (photo && photo.tagName === "VIDEO") {
      photo.style.animation =
        state === "speaking"  ? "wtai-speak-bob .42s ease-in-out infinite alternate" :
        state === "listening" ? "wtai-listen-pulse 1.2s ease-in-out infinite" :
                                "wtai-breathe 5s ease-in-out infinite";
    }
...
@keyframes wtai-blink-eye{0%,93%,100%{transform:scaleY(0)}95.5%,96.5%{transform:scaleY(1)}97%{transform:scaleY(0.08)}98.5%{transform:scaleY(0.92)}}
  ...
  function _initMouthCanvas(photoEl) {
    const wrap = document.getElementById("wtai-photo-wrap");
    if (!wrap) return;
    if (!_mouthCanvas) _mouthCanvas = document.getElementById("wtai-mouth-cv");
  ...
  /**
   * Draw mouth with full 5-viseme blending.
   *
   * Each viseme contributes a distinct shape:
   *   aa  – wide-open oval (peak F1, father/palm)
   *   ih  – narrow tall slit (bit/hit)
   *   ou  – small rounded pucker (boot/who)
   *   ee  – wide, thin rectangle — smile shape (beat/seat)
   *   oh  – medium round (bought/caught)
   *
   * The shapes are weighted by their respective viseme weights and
   * the results are blended via alpha compositing.
   */
  function _drawMouth() {
    if (!_mouthCtx || !_mouthCanvas) return;
    const ctx = _mouthCtx;
    const W = _mouthCanvas.width, H = _mouthCanvas.height;
    ctx.clearRect(0, 0, W, H);

    const amp = _smoothAmp;
    if (amp < 0.02) return;

    const cx = W * 0.50;
    const cy = H * 0.80;           // 80 % down = correct mouth row on head-shots
    const { r, g, b } = _skinColor;

    // ── Base dimensions (scaled by overall amplitude) ──────────────────────
    const baseW = W * 0.26;        // half-width of mouth zone
    const baseH = H * 0.068;       // resting lip height

    // ── Per-viseme shape parameters ────────────────────────────────────────
    // Each viseme returns { openW, openH } describing an inner cavity ellipse
    // that is blended into the final draw.
    const wAA = _vw.aa;
    const wIH = _vw.ih;
    const wOU = _vw.ou;
    const wEE = _vw.ee;
    ...
    // Effective opening dimensions = weighted sum of viseme contributions
    //   AA: wide & tall   IH: narrow & tall   OU: narrow & short (pucker)
    //   EE: wide & short  OH: medium-wide & medium-tall
    const openW = baseW * (
      wAA * 0.85 +
      wIH * 0.32 +
      wOU * 0.22 +
      wEE * 0.92 +
      wOH * 0.60 +
      amp * 0.45   // amplitude floor so mouth still opens when no dominant viseme
    );
    const openH = baseH * (
      wAA * 4.2 +
      wIH * 4.8 +
    ...
    // Corner pull for EE (smile stretch) and OU (pucker = negative)
    const smilePull  = wEE * baseW * 0.18;
    const puckerPull = wOU * baseW * 0.12;
    const cornerMod  = smilePull - puckerPull;

    // Lip height modifications per viseme
    const topLip = -openH * 0.44 - puckerPull * 0.4;
    const btmLip =  openH * 0.74 + puckerPull * 0.4;

    // ── 1. Skin-tone base (erases closed-lip pixels from photo) ──────────
    const sg = ctx.createRadialGradient(cx, cy, 0, cx, cy, baseW * 0.68);
    sg.addColorStop(0,   `rgba(${~~r},${~~g},${~~b},1)`);
    sg.addColorStop(0.6, `rgba(${~~r},${~~g},${~~b},0.92)`);
    sg.addColorStop(1,   `rgba(${~~r},${~~g},${~~b},0)`);
    ctx.fillStyle = sg;
    ctx.beginPath();
    ctx.ellipse(cx, cy, baseW * 0.68, baseH * 0.56 + openH * 0.6, 0, 0, Math.PI * 2);
    ctx.fill();

    // ── 2. Inner mouth cavity ─────────────────────────────────────────────
    if (openH > 0.5) {
      const cavH = openH * 0.52 + 0.5;
      const cavW = Math.min(openW * 0.88, baseW * 0.82);
      const mg = ctx.createRadialGradient(cx, cy + openH * 0.12, 0, cx, cy + openH * 0.12, cavW);
      mg.addColorStop(0,   "rgba(14,5,5,1)");
      mg.addColorStop(0.65,"rgba(28,8,8,0.97)");
      mg.addColorStop(1,   "rgba(50,14,14,0.18)");
      ctx.fillStyle = mg;
      ctx.beginPath();
      ctx.ellipse(cx, cy + openH * 0.12, cavW, cavH, 0, 0, Math.PI * 2);
      ctx.fill();

      // ── 3. Teeth (visible above amp 0.20) ──────────────────────────────
      const tAmp = Math.max(0, (amp - 0.20) / 0.80);
      if (tAmp > 0.05 && wOU < 0.6) {    // no teeth when very puckered
        const tw = Math.min(openW * 0.90, baseW * 0.84);
        const th = Math.min(openH * 0.36, baseH * 0.92) * tAmp;
        const ty = cy - th * 0.52;
        const tg = ctx.createLinearGradient(cx, ty, cx, ty + th);
        tg.addColorStop(0, "rgba(253,250,246,0.94)");
        tg.addColorStop(1, "rgba(238,232,222,0.86)");
        ctx.fillStyle = tg;
        ctx.beginPath(); ctx.rect(cx - tw / 2, ty, tw, th); ctx.fill();
        ctx.strokeStyle = "rgba(195,188,174,0.28)"; ctx.lineWidth = 0.7;
        for (let i = 1; i <= 3; i++) {
          const tx = cx - tw / 2 + (tw / 4) * i;
          ctx.beginPath(); ctx.moveTo(tx, ty + th * 0.06); ctx.lineTo(tx, ty + th * 0.88); ctx.stroke();
        }
      }
    }

    // ── 4. Upper lip (cupid's bow, with smile/pucker modification) ────────
    const ulr = ~~(r * 0.70), ulg = ~~(g * 0.56), ulb = ~~(b * 0.54);
    const topY = cy - baseH * 0.5 + topLip;
    const ucY  = cy - openH * 0.38;
    const ulHW = baseW * 0.47 + cornerMod;   // outer corner x-offset
    ctx.fillStyle = `rgba(${ulr},${ulg},${ulb},0.94)`;
    ctx.beginPath();
    ctx.moveTo(cx - ulHW, ucY + baseH * 0.1);
    ctx.bezierCurveTo(
      cx - baseW * 0.38, ucY - baseH * 0.06,
      cx - baseW * 0.21, topY + baseH * 0.03,
      cx - baseW * 0.09, topY + baseH * 0.14
    );
    ctx.bezierCurveTo(
      cx - baseW * 0.04, topY + baseH * 0.2,
      cx + baseW * 0.04, topY + baseH * 0.2,
      cx + baseW * 0.09, topY + baseH * 0.14
    );
    ctx.bezierCurveTo(
      cx + baseW * 0.21, topY + baseH * 0.03,
      cx + baseW * 0.38, ucY - baseH * 0.06,
      cx + ulHW,         ucY + baseH * 0.1
    );
    ctx.bezierCurveTo(cx + baseW * 0.34, ucY + baseH * 0.22, cx - baseW * 0.34, ucY + baseH * 0.22, cx - ulHW, ucY + baseH * 0.1);
    ctx.closePath(); ctx.fill();
    // Philtrum highlight
    ctx.fillStyle = `rgba(${Math.min(255,ulr+30)},${Math.min(255,ulg+22)},${Math.min(255,ulb+22)},0.30)`;
    ctx.beginPath(); ctx.ellipse(cx, topY + baseH * 0.22, baseW * 0.09, baseH * 0.045, 0, 0, Math.PI * 2); ctx.fill();

    // ── 5. Lower lip (fuller, shinier) ────────────────────────────────────
    const llr = ~~(r * 0.74), llg = ~~(g * 0.60), llb = ~~(b * 0.58);
    const lcY  = cy + openH * 0.40;
    const btmY = cy + baseH * 0.5 + btmLip;
    const llHW = baseW * 0.47 + cornerMod;
    ctx.fillStyle = `rgba(${llr},${llg},${llb},0.94)`;
    ctx.beginPath();
    ctx.moveTo(cx - llHW, lcY - baseH * 0.08);
    ctx.bezierCurveTo(cx - baseW * 0.41, lcY + baseH * 0.11, cx - baseW * 0.19, btmY - baseH * 0.04, cx, btmY);
    ctx.bezierCurveTo(cx + baseW * 0.19, btmY - baseH * 0.04, cx + baseW * 0.41, lcY + baseH * 0.11, cx + llHW, lcY - baseH * 0.08);
    ctx.bezierCurveTo(cx + baseW * 0.35, lcY - baseH * 0.13, cx - baseW * 0.35, lcY - baseH * 0.13, cx - llHW, lcY - baseH * 0.08);
    ctx.closePath(); ctx.fill();
    // Lower-lip highlight
  ...
  function buildUI() {
    ...
    avatarEl = document.getElementById("wtai-photo-wrap");
    ...
    sendBtn.onclick = () => {
      if (streaming && !inputEl.value.trim()) _interruptStream();
  ...
  function render() {
    ...
    if (msgs.length === 0) {
      ...
      msgsEl.querySelectorAll(".wtai-chip").forEach(b => { b.onclick = () => send(b.textContent); });
      return;
    }
    ...
    msgsEl.innerHTML = msgs.map(m => {
      ...
      return `<div class="wtai-mrow bot"><div class="wtai-mav">${ICO.bot}</div><div><div class="wtai-bub">${body}</div>${srcs}</div></div>`;
    }).join("");
  ...
  async function send(text) {
    ...
    try {
      ...
      const last = msgs[msgs.length - 1];
      if (last?.streaming) {
        last.content   = fullAnswer || last.content || "I couldn't generate a response.";
  ...
  // ── TTS — sentence-pipelined with pre-fetching ────────────────────────────────
  ...
  function toggleVoice() {
    ...
    // Instant feedback before permission dialog
    listening = true;
    _setMicState(true);
    ...
    recognition.onstart  = () => { listening = true; _setMicState(true); };
    recognition.onend    = () => { listening = false; _setMicState(false); setAvatarState(streaming ? "thinking" : "idle"); inputEl.placeholder = "Ask anything…"; };
    recognition.onerror  = (e) => {
      listening = false; _setMicState(false); setAvatarState("idle"); inputEl.placeholder = "Ask anything…";
      if (e.error === "not-allowed") { setStatus("Mic blocked — allow in browser settings"); setTimeout(() => setStatus("Ask about this site"), 4000); }
    };
    recognition.onresult = e => { const t = e.results[0]?.[0]?.transcript; if (t) send(t); };
  ...
  async function init(options) {
    ...
    console.log(`[WebTalkAI] v${V} init — real-time lip-sync`, { apiUrl: cfg.apiUrl, theme: themeKey });

    try {
      const t0  = performance.now();
      const res = await fetch(`${cfg.apiUrl}/api/v1/widget/config?api_key=${encodeURIComponent(cfg.apiKey)}`);
      if (!res.ok) { console.error(`[WebTalkAI] Bootstrap failed HTTP ${res.status}`); return; }
