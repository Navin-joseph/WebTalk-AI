"use strict";(()=>{var j="2.1.4",J="https://webtalk-ai.onrender.com",F="wss://webtalk-ai.onrender.com";console.log("%c[WebTalkAI v"+j+"] %cText = REST \xB7 Voice = WebSocket (mic only)","background:#6366f1;color:#fff;padding:2px 6px;border-radius:3px;font-weight:bold","color:#6b7280");var O={purple:{primary:"#6366f1",accent:"#a855f7"},blue:{primary:"#3b82f6",accent:"#06b6d4"},green:{primary:"#10b981",accent:"#84cc16"},dark:{primary:"#1f2937",accent:"#6366f1"}},o,b=null,p=[],C,x=!1,k=!1,c=null,g=null,L=null,U=!1,_=null,T=null,y,l,S,m,P,B,W,R,f;function X(){try{let e="wtai_session_v2",t=localStorage.getItem(e);return t||(t=`s_${Math.random().toString(36).slice(2,11)}_${Date.now()}`,localStorage.setItem(e,t)),t}catch(e){return`s_${Date.now()}`}}function v(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}function Y(e){try{let t=new URL(e),i=t.pathname.length>18?t.pathname.slice(0,18)+"\u2026":t.pathname;return t.hostname.replace(/^www\./,"")+(i==="/"?"":i)}catch(t){return e.slice(0,30)}}function G(){let e=o.theme,t=o.position==="bottom-right"?"right":"left",i=`
...
@media (max-width: 480px) {
  ...
  `.trim(),a=document.createElement("style");a.id="wtai-styles",a.textContent=i,document.head.appendChild(a)}function Z(){y=document.createElement("button"),y.className="wtai-w wtai-launcher",y.setAttribute("aria-label","Open chat"),y.innerHTML='<svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12c0 1.86.5 3.6 1.38 5.1L2 22l4.9-1.38C8.4 21.5 10.14 22 12 22c5.52 0 10-4.48 10-10S17.52 2 12 2zm0 18c-1.65 0-3.27-.45-4.7-1.3l-.34-.2-3.04.86.85-3.03-.21-.34C3.45 14.27 3 12.65 3 11c0-4.97 4.03-9 9-9s9 4.03 9 9-4.03 9-9 9z"/><circle cx="8" cy="12" r="1.4"/><circle cx="12" cy="12" r="1.4"/><circle cx="16" cy="12" r="1.4"/></svg>',y.onclick=z,document.body.appendChild(y),l=document.createElement("div"),l.className="wtai-w wtai-panel",l.innerHTML=`
    <div class="wtai-head">
  ...
  `,document.body.appendChild(l),S=l.querySelector(".wtai-msgs"),m=l.querySelector(".wtai-in"),B=l.querySelector(".wtai-send"),P=l.querySelector(".wtai-mic"),W=l.querySelector(".wtai-sugs"),R=l.querySelector(".wtai-voice"),f=l.querySelector(".wtai-vstat"),l.querySelector(".wtai-close").onclick=z,B.onclick=()=>D(),m.addEventListener("keydown",e=>{e.key==="Enter"&&!k&&D()}),P&&(P.onclick=ne),l.querySelector('[data-act="cancel"]').onclick=()=>M(!0),l.querySelector('[data-act="stop"]').onclick=()=>M(!1)}function z(){x=!x,l.classList.toggle("open",x),x&&(m.focus(),setTimeout(()=>S.scrollTo({top:S.scrollHeight,behavior:"smooth"}),50))}function A(){S.innerHTML=p.map(e=>{let t='<svg viewBox="0 0 24 24"><path d="M20 9V7c0-1.1-.9-2-2-2h-3c0-1.66-1.34-3-3-3S9 3.34 9 5H6c-1.1 0-2 .9-2 2v2c-1.66 0-3 1.34-3 3s1.34 3 3 3v4c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2v-4c1.66 0 3-1.34 3-3s-1.34-3-3-3zM7.5 11.5c0-.83.67-1.5 1.5-1.5s1.5.67 1.5 1.5S9.83 13 9 13s-1.5-.67-1.5-1.5zM16 17H8v-2h8v2zm-1-4c-.83 0-1.5-.67-1.5-1.5S14.17 10 15 10s1.5.67 1.5 1.5S15.83 13 15 13z"/></svg>',i='<svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>',a=e.sources&&e.sources.length&&!e.streaming?`<div class="wtai-srcs">${e.sources.slice(0,4).map(h=>`<a class="wtai-src" href="${v(h)}" target="_blank" rel="noopener">${v(Y(h))}</a>`).join("")}</div>`:"";if(e.role==="user")return`<div class="wtai-msg user"><div class="wtai-bub">${v(e.content)}</div></div>`;let s=e.error?"wtai-msg assistant err":"wtai-msg assistant",d=e.error?"wtai-mbot err":"wtai-mbot",r=e.streaming&&e.content?'<span class="wtai-caret"></span>':"",n=e.streaming&&!e.content?'<div class="wtai-typing"><span></span><span></span><span></span></div>':`${v(e.content)}${r}`;return`<div class="${s}">
      <div class="${d}">${e.error?i:t}</div>
      <div style="max-width:75%"><div class="wtai-bub">${n}</div>${a}</div>
    </div>`}).join(""),S.scrollTo({top:S.scrollHeight,behavior:"smooth"})}function q(){if(p.length>1||k){W.innerHTML="";return}let e=["What is this site about?","What do you offer?","How can I get in touch?"];W.innerHTML=e.map(t=>`<button class="wtai-sug">${v(t)}</button>`).join(""),W.querySelectorAll(".wtai-sug").forEach((t,i)=>{t.onclick=()=>D(e[i])})}var K=6e4;async function D(e){var d,r;let t=(e!=null?e:m.value).trim();if(!t||k)return;m.value="",p.push({role:"user",content:t}),p.push({role:"assistant",content:"",streaming:!0}),k=!0,B.disabled=!0,m.disabled=!0,A(),q();let i=new AbortController,a=setTimeout(()=>{console.warn("[WebTalkAI] Request timed out after",K,"ms"),i.abort()},K),s=performance.now();console.log("[WebTalkAI] \u2192 REST POST /widget/chat",{message:t,conversation_id:C});try{let n=await fetch(`${o.apiUrl}/api/v1/widget/chat`,{method:"POST",headers:{"Content-Type":"application/json","X-API-Key":o.apiKey},body:JSON.stringify({api_key:o.apiKey,message:t,conversation_id:C}),signal:i.signal});clearTimeout(a);let h=Math.round(performance.now()-s);if(console.log("[WebTalkAI] \u2190 status",n.status,"in",h,"ms"),!n.ok){let $="";try{$=await n.text()}catch(oe){}console.error("[WebTalkAI] HTTP",n.status,$);let E=`Server returned ${n.status}.`;throw n.status===401?E="Invalid API key.":n.status===404?E="Endpoint not found.":n.status===429?E="Too many requests. Please wait a moment.":n.status>=500&&(E="The AI is having trouble. Please try again."),new Error(E)}let u=await n.json();console.log("[WebTalkAI] response:",{len:(d=u.response)==null?void 0:d.length,sources:(r=u.sources)==null?void 0:r.length});let w=u.response||u.answer||"I couldn't generate a response. Please try again.",N=u.sources||[],I=p[p.length-1];I&&(I.content=w,I.sources=N,I.streaming=!1),A(),o.ttsAutoPlay&&w&&te(w).catch($=>console.warn("[WebTalkAI] TTS failed:",$))}catch(n){clearTimeout(a);let h=Math.round(performance.now()-s);console.error("[WebTalkAI] Chat failed after",h,"ms:",n);let u="Something went wrong. Please try again.";n instanceof DOMException&&n.name==="AbortError"?u="The AI is starting up \u2014 please wait a moment and try again. (The server sleeps when idle on free hosting.)":n instanceof TypeError&&n.message==="Failed to fetch"?u="Couldn't reach the server. Please check your connection.":n instanceof Error&&(u=n.message);let w=p[p.length-1];w&&w.streaming?(w.content=u,w.streaming=!1,w.error=!0):p.push({role:"assistant",content:u,error:!0}),A()}finally{k=!1,B.disabled=!1,m.disabled=!1,m.focus()}}function Q(){if(!_){let e=window.AudioContext||window.webkitAudioContext;_=new e}return _}function ee(){if(T){try{T.stop()}catch(e){}T=null}}async function V(e){if(e.byteLength===0){console.warn("[WebTalkAI] Empty audio buffer \u2014 skipping playback");return}ee();let t=Q();t.state==="suspended"&&await t.resume();let i=await t.decodeAudioData(e.slice(0)),a=t.createBufferSource();return a.buffer=i,a.connect(t.destination),new Promise(s=>{a.onended=()=>{T===a&&(T=null),s()},T=a,a.start(0)})}var H=null;async function te(e){if(H!==!1)try{let t=await fetch(`${o.apiUrl}/api/v1/widget/tts`,{method:"POST",headers:{"Content-Type":"application/json","X-API-Key":o.apiKey},body:JSON.stringify({text:e})});if(t.ok){let a=await t.arrayBuffer();if(a.byteLength>0){console.log("[WebTalkAI] TTS (ElevenLabs):",a.byteLength,"bytes"),H=!0,await V(a);return}}let i="";try{i=JSON.stringify(await t.json())}catch(a){i=await t.text().catch(()=>"")||""}console.warn("[WebTalkAI] Server TTS unavailable (HTTP "+t.status+"); using browser voice for the rest of this session."),i&&console.warn("[WebTalkAI] Server detail:",i.slice(0,200)),H=!1}catch(t){console.warn("[WebTalkAI] Server TTS error, switching to browser voice:",t),H=!1}ae(e)}function ae(e){if(typeof window=="undefined"||!("speechSynthesis"in window)){console.warn("[WebTalkAI] Browser TTS not supported in this browser");return}window.speechSynthesis.cancel();let t=!1,i=()=>{var r;if(t)return;t=!0;let a=new SpeechSynthesisUtterance(e);a.rate=1,a.pitch=1,a.volume=1;let s=window.speechSynthesis.getVoices(),d=s.find(n=>/Google US English|Samantha|Karen|Daniel|Microsoft.*Natural/i.test(n.name))||s.find(n=>n.lang&&n.lang.toLowerCase().startsWith("en"));d&&(a.voice=d),console.log("[WebTalkAI] TTS (browser):",((r=a.voice)==null?void 0:r.name)||"default"),window.speechSynthesis.speak(a)};window.speechSynthesis.getVoices().length===0?(window.speechSynthesis.onvoiceschanged=()=>{window.speechSynthesis.onvoiceschanged=null,i()},setTimeout(i,250)):i()}async function ne(){if(!o.voiceEnabled||!b||k)return;R.classList.add("active"),f.textContent="Connecting\u2026",U=!1;try{L=await navigator.mediaDevices.getUserMedia({audio:!0})}catch(i){f.textContent="Microphone access denied",setTimeout(()=>M(!0),1500);return}let e=`${o.wsUrl}/ws/voice/${b.client_id}?session_id=${encodeURIComponent(C)}&api_key=${encodeURIComponent(o.apiKey)}`;try{c=new WebSocket(e)}catch(i){f.textContent="Connection failed",setTimeout(()=>M(!0),1500);return}let t=[];c.onopen=()=>{f.textContent="Listening\u2026",g=new MediaRecorder(L,{mimeType:"audio/webm"}),g.ondataavailable=i=>{if(!c||c.readyState!==WebSocket.OPEN||i.data.size===0)return;let a=new FileReader;a.onload=()=>{let s=a.result.split(",")[1];c.send(JSON.stringify({type:"audio_chunk",data:s}))},a.readAsDataURL(i.data)},g.start(300)},c.onmessage=i=>{let a=JSON.parse(i.data);if(a.type==="transcript"&&a.text)p.push({role:"user",content:a.text}),A();else if(a.type==="answer_text")p.push({role:"assistant",content:a.text}),f.textContent="Speaking\u2026",A();else if(a.type==="audio_chunk"){let s=atob(a.data),d=new Uint8Array(s.length);for(let r=0;r<s.length;r++)d[r]=s.charCodeAt(r);t.push(d)}else if(a.type==="audio_end"){let s=t.reduce((n,h)=>n+h.length,0),d=new Uint8Array(s),r=0;for(let n of t)d.set(n,r),r+=n.length;t.length=0,V(d.buffer).then(()=>{U||(f.textContent="Listening\u2026",g&&g.state==="inactive"&&g.start(300))}).catch(n=>console.warn("[WebTalkAI] voice playback failed:",n))}else a.type==="error"&&(f.textContent="Error: "+a.message,setTimeout(()=>M(!0),1800))},c.onerror=()=>{f.textContent="Voice connection error"},c.onclose=()=>{U||(f.textContent="Disconnected")}}function M(e){U=e,R.classList.remove("active"),g&&g.state!=="inactive"&&g.stop(),!e&&c&&c.readyState===WebSocket.OPEN?(f.textContent="Thinking\u2026",c.send(JSON.stringify({type:"audio_end"}))):c&&(c.close(),c=null),L&&(L.getTracks().forEach(t=>t.stop()),L=null),g=null}async function ie(e){if(!e||!e.apiKey){console.error("[WebTalkAI] apiKey is required");return}if(document.getElementById("wtai-styles")){console.warn("[WebTalkAI] Already initialized");return}let t=e.theme&&O[e.theme]?e.theme:"purple";o={apiKey:e.apiKey,apiUrl:e.apiUrl||J,wsUrl:e.wsUrl||F,position:e.position==="bottom-left"?"bottom-left":"bottom-right",theme:O[t],greeting:e.greeting||"Hi! How can I help you today?",voiceEnabled:e.voiceEnabled!==!1,ttsAutoPlay:e.ttsAutoPlay===!0},C=X(),console.log("[WebTalkAI] init v"+j,{apiUrl:o.apiUrl,wsUrl:o.wsUrl,theme:e.theme||"purple",position:o.position,voiceEnabled:o.voiceEnabled,sessionId:C});let i=performance.now();try{let a=await fetch(`${o.apiUrl}/api/v1/widget/config?api_key=${encodeURIComponent(o.apiKey)}`),s=Math.round(performance.now()-i);if(!a.ok){console.error(`[WebTalkAI] Bootstrap failed: HTTP ${a.status} after ${s}ms. Check that the API key is valid and that ${o.apiUrl} is reachable.`);return}b=await a.json(),console.log("[WebTalkAI] connected in",s,"ms \u2014 tenant:",b==null?void 0:b.company_name)}catch(a){console.error("[WebTalkAI] Bootstrap failed (network):",a);return}G(),Z(),p=[{role:"assistant",content:o.greeting}],A(),q()}window.WebTalkAI={init:ie,open:()=>{x||z()},close:()=>{x&&z()},version:j};})();
